...
public class Advice implements AsmVisitorWrapper.ForDeclaredMethods.MethodVisitorWrapper, Implementation {
    ...
    public interface ArgumentHandler {
        ...
        interface ForInstrumentedMethod extends ArgumentHandler {
            ...
            /**
             * Returns {@code true} if the original arguments are copied.
             *
             * @return {@code true} if the original arguments are copied.
             */
        ...
        enum Factory {
            ...
            COPYING {
                ...
                protected ForInstrumentedMethod resolve(MethodDescription instrumentedMethod, TypeDefinition enterType) {
                    ...
                    return instrumentedMethod.isStatic() && instrumentedMethod.getParameters().isEmpty()
                            ? new ArgumentHandler.Factory.Simple(instrumentedMethod, enterType)
                            : new ArgumentHandler.Factory.Copying(instrumentedMethod, enterType);
                }
            };
    ...
    protected interface StackMapFrameHandler {
        ...
        abstract class Default implements ForInstrumentedMethod {
             ...
             * @param instrumentedMethod The instrumented method.
             * @param copyArguments      {@code true} if the original arguments are copied.
             * @param noExit             {@code true} if no exit advice is applied.
             * @param enterTypes         A list of virtual method arguments that are available before the instrumented method is executed.
             * @param exitTypes          A list of virtual method arguments that are available after the instrumented method has completed.
            ...
            protected static class WithTrivialTranslation extends Default {
                 ...
                 * @param expandFrames       {@code true} if the meta data handler is expected to expand its frames.
                 */
                protected WithTrivialTranslation(TypeDescription instrumentedType,
                                                 MethodDescription instrumentedMethod,
                                                 List<? extends TypeDescription> exitTypes,
                                                 boolean expandFrames) {
            ...
            protected class ForAdvice implements StackMapFrameHandler.ForAdvice {
                ...
                public void injectReturnFrame(MethodVisitor methodVisitor) {
                    if (!expandFrames && currentFrameDivergence == 0) {
                        if (endTypes.isEmpty() || adviceMethod.getReturnType().represents(void.class)) {
                            methodVisitor.visitFrame(Opcodes.F_SAME, EMPTY.length, EMPTY, EMPTY.length, EMPTY);
                        } else {
    ...
    protected abstract static class AdviceVisitor extends ExceptionTableSensitiveMethodVisitor implements Dispatcher.Bound.SkipHandler {
        ...
        protected AdviceVisitor(MethodVisitor methodVisitor,
            ...
            methodSizeHandler = MethodSizeHandler.Default.of(instrumentedMethod, argumentHandler.isCopyArguments(), enterTypes, exitTypes, writerFlags);
            ...
            protected void onUserEnd() {
                ...
                if (doesReturn) {
                    ...
                    if (!returnType.equals(Type.VOID_TYPE)) {
                        mv.visitVarInsn(returnType.getOpcode(Opcodes.ISTORE), argumentHandler.returned());
                    }
                }
                onUserReturn();
                methodExit.apply();
                ...
                if (returnType.equals(Type.VOID_TYPE)) {
                    ...
                    mv.visitVarInsn(returnType.getOpcode(Opcodes.ILOAD), argumentHandler.returned());
